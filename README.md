...
- 💃 **持續大動作 / 跳舞**：偵測肩膀、手腕、髖部的移動能量（`danceEnergy`），超過門檻進入「跳舞中」狀態，觸發**沿著身體外框飄動**的小精靈魔法光點（`auraPoints`/`auraOuterPoints`，見下方「小精靈魔法光點」）；跳超過 0.7 秒還會自動展開一次宇宙星空模式
...
跳舞時的光點**沿著身體外框飄動**，不是黏在胸口的一顆球：

- `BODY_OUTLINE_IDS` 把頭、肩、手肘、手腕、髖、膝、腳踝連成一圈環繞身體的路徑（`buildBodyOutline`），每顆光點沿著這條路徑慢慢繞行（`pointOnOutline`），並往外浮動一小段距離
- 光點會呼吸（忽近忽遠飄離身體外框）、閃爍（每顆亮度隨時間明暗，`twinkle`），比較有螢火蟲/小精靈粉塵的感覺，不是死板的固定光環
- 兩層光點：內層貼身、外層更遠更飄，營造出「魔法光塵環繞全身」而不是單一光球
